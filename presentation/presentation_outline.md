# RepoMentor 最终展示 PPT 结构提纲

## 设计原则

- 页数：9 页以内。
- 场景：比赛或课程最终展示，重点说明系统设计、工程实现和迭代过程。
- 风格：浅灰或白底工程科技风，主色使用深蓝、深紫和青色。
- 结构：每页一个核心结论，每页 3 到 5 个要点，尽量用流程图、分层图、卡片和时间线表达。
- 重点：系统架构、Agent 工作流、Evidence 机制、用户页与调试页分离、压力测试结果、后续路线。
...
内容：
- RepoMentor：结构感知型 GitHub 仓库学习 Agent。
- 从陌生仓库到可执行学习路径与入门贡献任务。
- Repository Intelligence Graph。
- Evidence-grounded Agent。

视觉：
- 左侧标题与一句话定位。
- 右侧使用 GitHub URL、Repository Graph、Agent、用户页和调试页组成的简洁系统图。

## 第 2 页：问题背景与任务目标

核心结论：新贡献者的难点不是没有信息，而是缺少可信、可执行的阅读路径。

左右对照：
- 痛点：文件多、入口不清、命令分散、Issue 不适合新人、回答容易幻觉。
- RepoMentor 能力：快速理解项目目标、学习关键模块、理解开发流程、找到入门任务、降低学习成本。
...
- External：GitHub API、DeepSeek 或 Mock Provider。
...
左侧：
- 用户页截图。
- 标注仓库概览、学习路径、开发流程、推荐任务、QA。
- 标注“设计依据”：学习路径、Contribution Funnel、新手友好度、EvidenceBuilder。

右侧：
- 调试页截图。
- 标注 Agent Flow、Worker Outputs、Data Layer、SelfCheck、FinalAnswer JSON。
- 标注“执行轨迹”：evidence、trace、raw JSON。
...
核心结论：最终提交由 PPT、可本地运行代码和完整交互轨迹组成。

内容：
- 本地运行：后端 8000，前端 5173。
- 系统展示：PPT 不超过 10 页，突出设计亮点和迭代过程。
- 完整交互轨迹：Codex、Claude Code、`docs/interaction_trace.md`。
- 项目文档：README、testing_summary、upgrade_plan、design_basis。
