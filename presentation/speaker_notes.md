...
这一页讲学习路径设计依据。RepoMentor 不把学习路径做成简单的读 README、读源码，而是拆成目标、操作、观察、自检四步。目标说明这一步要学会什么，操作给出文件和命令，观察告诉用户应该看到什么结果，自检要求用户能解释回去。底部的小例子体现从运行示例脚本到追踪入口、定位模块、对应测试的渐进式脚手架。
