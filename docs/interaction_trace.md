...
- 调试页展示 Agent Flow、Worker Outputs、Data Layer、Retrieval Trace、Shared Memory、SelfCheck 和 FinalAnswer JSON。
...
- open issue 为 0 时生成 internal first tasks，并明确标记不是 GitHub Issue。

### 阶段 4：Evidence 与 SelfCheck

- 改进 setup_run 问答，让回答直接给出启动方式、示例命令、测试命令、风险和证据来源。
- Shared Working Memory 自动合并 Worker evidence。
...
- 生成最终展示 PPT 和 PDF。
- 整理 speaker notes、testing summary、upgrade plan 和 design basis。
- 保留完整交互轨迹说明。
- 根据最终要求，公开仓库移除录屏脚本、在线部署占位说明和容器化一键运行文件。
...
7. 生成最终展示材料，满足本地可运行、系统展示和完整交互轨迹。
8. 开源到 GitHub 后，按要求移除公开仓库中的 Demo/部署交付内容，并统一 README 为中文。
...
压力测试没有发现系统崩溃。失败点主要是 evidence 覆盖不足，说明回答链路可运行，但证据注入和 EvidenceBuilder 仍需加强。因此下一阶段优先级不是继续堆功能，而是先补 EvidenceBuilder。
